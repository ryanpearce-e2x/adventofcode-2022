...
export default class Day4 extends Day implements IDay {
  ...
  getOverlapCount(matchType: MatchType) {
    ...
    for (let i = 0; i < this.assignmentPairs.length; i++) {
      ...
      const [array1, array2] = [
        this.createRange(lowerBound1, upperBound1),
        this.createRange(lowerBound2, upperBound2),
      ];
      const contains =
        this.arrayContains(array1, array2, matchType) ||
        this.arrayContains(array2, array1, matchType);
