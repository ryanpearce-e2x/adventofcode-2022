...
export default class Day2 extends Day implements IDay {
  ...
  getResponseMove = (move: Move, condition: string, ruling: Ruling): Move => {
    if (condition === 'X') {
      //Lose
      return Object.keys(ruling).find(
        (key) => ruling[key as Move].beats !== move && key !== move,
      ) as Move;
    } else if (condition === 'Z') {
      //Win
  ...
  playGame(move1: string, move2: string, ruling: Ruling, part: number) {
    ...
    const realMove2 =
      part === 1 ? this.getRealMove(move2, ruling) : this.getResponseMove(realMove1, move2, ruling);
